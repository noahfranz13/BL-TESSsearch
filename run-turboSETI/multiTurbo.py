...
def splitRun(nnodes, debug, t, outDir, splicedonly, unsplicedonly, sqlTable, slowdebug=False):
    ...
    for ii, node in zip(ii2D, cn):
        ...
        if len(ii) != 0:
            ...
            if debug:
                cmd = ['ssh', '-o ExitOnForwardFailure=True', node, f"source {condaenv} runTurbo ; source {varPath} ; python3 {cwd}/wrapTurbo.py --ii '{ii.tolist()}' --timer {t} --outdir {outDir} --test {debug} --sqlTable {sqlTable}"]

            else:
                cmd = ['ssh', '-o ExitOnForwardFailure=True',  node, f"source {condaenv} runTurbo ; source {varPath} ; python3 {cwd}/wrapTurbo.py --ii '{ii.tolist()}' --timer {t} --outdir {outDir} --sqlTable {sqlTable}"]
